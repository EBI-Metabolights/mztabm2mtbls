    ...
    def update(self, mztab_model: MzTab, mtbls_model: MetabolightsStudyModel):
            ...
            if sv.factors:
                for factor in sv.factors:
                    if factor.name:
                        ...
                        factor_tuple = (factor.cv_label, factor.cv_accession, factor.name, factor.value)
                        if sv.assay_refs:
                            assay_refs = [assays_map[x] for x in sv.assay_refs if x in assays_map]
                            if assay_refs:
                                sample_ref_ids = [x.sample_ref for x in assay_refs if x.sample_ref in samples_map]
        ...
        for characteristics_name in custom_characteristics:
            ...
            if characteristics_name not in ("Disease", "Cell type"):
                ...
                selected_column_headers[name] = FieldMapDescription(field_name=name, map_from="name")
                
        for factor_value_name in factor_value_names:
            name = f"Factor Value[{factor_value_name}]"
            selected_column_headers[name] = FieldMapDescription(field_name=name, map_from="value")
            ...
            for param in sample.custom:
                if param.name and  param.name in custom_characteristics:
                    ...
                    if header in selected_column_headers:
                        ...
                        samples_file.table.data[column_name][row_idx] = sanitise_data(param.value)
                    ...
                    if header in selected_column_headers:
                        ...
                        if val:
                            if samples_file.table.data[column_name][row_idx]:
                                samples_file.table.data[column_name][row_idx] += f";{val}"
                            else:
